...
useEffect(()=>{
    function removeDuplicates(arr, prop) {
        ...
        return arr.reduce((uniqueArray, item) => {
            const itemId = item.card.info[prop];
            if (!item.card.info.duplicateCount) {
                
                item.card.info.duplicateCount = 0; // Initialize duplicate count for each unique item
                uniqueArray.push(item);
            } else {
                // Increment duplicate count for each duplicate item
                const index = uniqueArray.findIndex(uniqueItem => uniqueItem.card.info[prop] === itemId);
                if (index !== -1) {
                    uniqueArray[index].duplicateCount++;
                
                    console.log("data",uniqueArray[index].duplicateCount);
                }
            }
            return uniqueArray;
        }, []);
    ...
    // Remove duplicates based on the 'id' property inside the 'info' object
    const uniqueArray = removeDuplicates(cartItems, 'id');
...
console.log(quantity);
    return (
        ...
        // </div>
        <div>
            {unique.map((item) => <div data-testid="foodItems" key={item.card.info.id}>
